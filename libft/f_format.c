...
static void		ft_fflags(char *sv, t_fprintf **pf)
{
	char f;
...
static void		ft_fsize_spec(char *sv, t_fprintf **pf)
{
	t_sequence	spec;
...
static void		ft_fminfld_presc(char *sv, t_fprintf **pf, va_list *pc, char f)
{
	while (*(++sv))
	{
...
void			ft_fs_spec(t_fprintf **pf, char *sv, va_list *pc)
{
	...
	ft_fflags((*pf)->spec, pf);
	ft_fsize_spec((*pf)->spec, pf);
	ft_fminfld_presc((*pf)->spec, pf, pc, 1);
}
...
int				ft_fprintf(char *sv, int fd, ...)
{
	...
	if ((pf = ft_s_fprintf_new()) == NULL)
		return (-1);
	pf->fd = fd;
	va_start(pc, fd);
	if (ft_strlen(sv) == 1 && *sv == '%')
		return (0);
	ft_fmaking_move(&pf, &pc, sv);
	va_end(pc);
	ft_fpuppies_killa(&pf);
	return (pf->lnpr);
}
