...
static void    ft_sti_1(t_corewar *data, t_carriage *src, char op_code, int index)
{
    if (op_code == 88 && src->arg[1] >= 0 && src->arg[1] <= REG_NUMBER && src->arg[2] >= 0 && src->arg[2] <= REG_NUMBER)
    {
        ...
        data->game_field[ft_if_negative(index)] = (unsigned int)src->reg[src->arg[1]] >> 24;
        data->game_field[ft_if_negative(index + 1)] = (unsigned int)src->reg[src->arg[1]] >> 16;
        data->game_field[ft_if_negative(index + 2)] = (unsigned int)src->reg[src->arg[1]] >> 8;
        data->game_field[ft_if_negative(index + 3)] = (unsigned int)src->reg[src->arg[1]];
    ...
    else if (op_code == 84 && src->arg[1] >= 0 && src->arg[1] <= REG_NUMBER && src->arg[2] >= 0 && src->arg[2] <= REG_NUMBER && src->arg[3] >= 0 && src->arg[3] <= REG_NUMBER)
    {
        ...
        data->game_field[ft_if_negative(index)] = (unsigned int)src->reg[1] >> 24;
        data->game_field[ft_if_negative(index + 1)] = (unsigned int)src->reg[1] >> 16;
        data->game_field[ft_if_negative(index + 2)] = (unsigned int)src->reg[1] >> 8;
        data->game_field[ft_if_negative(index + 3)] = (unsigned int)src->reg[1];
        ft_write_meta(data, src->name, index);
    }
}
