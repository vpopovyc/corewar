...
void	ft_st(t_corewar *data, t_carriage *src)
{
    if (data->game_field[src->position + 1] == 112)
    {
        ...
        if (src->arg[1] >= 0 && src->arg[1] <= REG_NUMBER)
        {
            ...
            data->game_field[ft_if_negative(src->arg[2])] = (unsigned char)(src->reg[src->arg[1]] >> 24);
            data->game_field[ft_if_negative(src->arg[2]) + 1] = (unsigned char)(src->reg[src->arg[1]] >> 16);
            data->game_field[ft_if_negative(src->arg[2]) + 2] = (unsigned char)(src->reg[src->arg[1]] >> 8);
            data->game_field[ft_if_negative(src->arg[2]) + 3] = (unsigned char)(src->reg[src->arg[1]]);
